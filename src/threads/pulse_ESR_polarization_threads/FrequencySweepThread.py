...
def add_delay(pulse_form, delay_dic):
    ...
    if int(laser_delay) > int(mw_delay):
        for each_pulse in pulse_form:
            # init temp pulse
            temp_pulse = [each_pulse[0]]
            if each_pulse[0] == 'Laser':
                temp_pulse.append(each_pulse[1])
                temp_pulse.append(each_pulse[2])
            elif each_pulse[0] == 'MicroWave':
                temp_pulse.append(str(int(each_pulse[1]) + int(laser_delay) - int(mw_delay)))
                temp_pulse.append(str(int(each_pulse[2]) + int(laser_delay) - int(mw_delay)))
            else:
                temp_pulse.append(str(int(each_pulse[1]) + int(laser_delay)))
                temp_pulse.append(str(int(each_pulse[2]) + int(laser_delay)))
            seq_with_delay.append(temp_pulse)
    else:
        for each_pulse in pulse_form:
            # init temp pulse
            temp_pulse = [each_pulse[0]]
            if each_pulse[0] == 'Laser':
                temp_pulse.append(str(int(each_pulse[1]) + int(mw_delay) - int(laser_delay)))
                temp_pulse.append(str(int(each_pulse[2]) + int(mw_delay) - int(laser_delay)))
            elif each_pulse[0] == 'MicroWave':
                temp_pulse.append(each_pulse[1])
                temp_pulse.append(each_pulse[2])
            else:
                temp_pulse.append(str(int(each_pulse[1]) + int(mw_delay)))
                temp_pulse.append(str(int(each_pulse[2]) + int(mw_delay)))
            seq_with_delay.append(temp_pulse)
